...
const argv: Arguments = yargs(hideBin(process.argv))
  .options({
    sourcePath: {
      type: "string",
      alias: "s",
      description: "The path you want proxied elsewhere",
    },
    proxyLocation: {
      type: "string",
      alias: "p",
      description: "The location to proxy the original source",
    },
    backupLocation: {
      type: "string",
      alias: "b",
      description: "The location to backup the existing proxy folder' contents",
    },
    restore: {
      type: "boolean",
      alias: "r",
      default: true,
      description: "Whether or not to restore the backup on close",
    },
    glob: {
      type: "string",
      alias: "g",
      description: "A glob to proxy to multiple directories at once",
    },
    ignoreDot: {
      type: "boolean",
      alias: "d",
      default: true,
      description: "Whether or not to ignore directories prefixed with a '.'",
    },
    watch: {
      type: "string",
      alias: "w",
      description: "A command to run when any source file changes",
    },
    watchDebounce: {
      type: "number",
      alias: "t",
      default: 5000,
      description:
        "An amount of time (in milliseconds) to wait after file updates before executing the 'watch' command",
    },
    ignore: {
      type: "string",
      alias: "i",
      description:
        "An ignore regex pattern, folders matching the pattern will not be watched, but will be proxied",
    },
  })
  .help()
  .normalize(["sourcePath", "proxyLocation", "backupLocation"])
  .demandOption(["sourcePath", "proxyLocation", "backupLocation"]).argv;

configureExitHandler();

let { proxyLocation, sourcePath, backupLocation } = argv;
...
const { glob, watch, watchDebounce, ignore, ignoreDot, restore } = argv;
...
(async function () {
  console.log(
    `Proxying the following locations:\n${JSON.stringify(
      paths.map((path) => path.proxy)
    )}`
  );
  const { shouldContinue } = await prompts([
    {
      message: "Proxy these folders?",
      name: "shouldContinue",
      type: "confirm",
      initial: true,
    },
...
function exitHandler(
  ...
  (async function () {
    ...
    if (restore) await proxyDirectoryService.delete(backupLocation);
